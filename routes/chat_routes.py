...
async def chat_completions(request: Request, user_info: Dict[str, Any] = Depends(get_current_user)):
    ...
    message_id = _resolve_message_id(messages)
...
async def handle_non_streaming_request(
    ...
    if model == GITBOOK_MODEL_NAME:
        limit = _sanitize_gitbook_limit(gitbook_options)
        try:
            result = await _run_gitbook_answer(user_message, limit)
        except ValueError as exc:
            error_response = {
                "error": {
                    "message": str(exc),
        ...
        response = _build_openai_response(session_id, model, result, user_id)
        return JSONResponse(content=response)

    return await _handle_general_non_stream(
        user_message,
        session_id,
        user_id,
        model,
...
async def _handle_general_non_stream(
    user_message: str,
    session_id: str,
    user_id: Optional[str],
    model: str,
    ...
    query_request = _build_query_request(agent_config, user_message, conversation_history)
    query_agent = QueryAgent()
    ...
    try:
        ...
        logger.error("Error during non-streaming request: %s", exc)
        error_response = {
            "error": {
    ...
    response = _build_openai_response(session_id, model, result_dict, user_id)
    return JSONResponse(content=response)
